...
import { FileText, Download, Eye, Sparkles, Settings, BookOpen, LayoutTemplate, Plus, X, Paperclip } from 'lucide-react';
                          ...
                          {predefinedTemplates.map((template) => (
                            <div
                              ...
                              onClick={() => {
                                ...
                                const totalQuestions = template.sections.reduce((sum, section) => sum + section.questionCount, 0);
                                const questionTypesFromTemplate = template.sections.map(section => section.name);
                                setFormData(prev => ({
                                  ...prev,
                                  questionCount: totalQuestions.toString(),
                                  questionTypes: questionTypesFromTemplate
                                }));
                                setIsPremadeTemplateOpen(false);
                                toast.success(`${template.name} selected and configuration updated`);
                              }}
                            >
